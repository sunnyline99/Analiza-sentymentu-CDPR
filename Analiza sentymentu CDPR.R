...
## Jeżeli nie została zainstalowana jedna z bibliotek należy użyć funkcji install.packages(*nazwa_biblioteki*)
...
#Czyszczenie tweetow + zamiana emotikonów na tekst przez wgranie z pliku + analiza emotek

Dane <- read.csv("*dostęp do ścieżki Old_data*")
...
Emotikony_db <- read.csv("*dostęp do ścieżki emoji_df.csv*")
Emotikony_sentyment <- read.csv("*dostęp do ścieżki Emoji_Sentiment_Data.csv*")
...
## emocjonalnosc wszystkich wypowiedzi w całym badanym okresie
...
## for loop żeby wyciagnac sentyment z kazdego dnia
...
## połączenie baz danych + analiza korelacji
